...
import type { SiteConfig, GalleryImage } from '../config/types';
import { apiImageCache } from '../utils/apiImages';

const config: SiteConfig = siteConfig as SiteConfig;

// Fallback gallery images configuration
const fallbackImages: GalleryImage[] = config.gallery.fallbackImages;

const HeroImage: React.FC = () => {
  const [currentIndex, setCurrentIndex] = React.useState(0);
  const [isTransitioning, setIsTransitioning] = React.useState(false);
  const [galleryImages, setGalleryImages] =
    React.useState<GalleryImage[]>(fallbackImages);

  // Load Instagram images on component mount
  React.useEffect(() => {
    const loadInstagramImages = async () => {
      ...
      try {
        const instagramImages = await apiImageCache.getImages(
          config.gallery.instagram.maxImages
        );

        if (instagramImages.length > 0) {
          setGalleryImages(instagramImages);
          console.log(`Loaded ${instagramImages.length} images from API`);
        } else {
          console.log('No Instagram images available, using fallback images');
        }
      } catch (error) {
